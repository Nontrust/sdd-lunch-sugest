...
### Requirement: 식당 추가
사용자는 웹 UI에서 상호명과 카테고리(한식/중식/일식/양식)를 입력하여 새 식당을 등록할 수 있어야 한다.
...
사용자는 등록된 모든 식당과 메뉴를 조회할 수 있어야 한다.
